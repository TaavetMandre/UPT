...
var times_of_day = [morning, day, evening, night]
var current_time

func cycle_current_time(): # changes the current time dictionary
	current_time = times_of_day.pop_front()
	times_of_day.append(current_time)

func cycle_time(time_dict: Dictionary, lerp_speed: float):
	if time_dict:
		# SUN
